...
export default class WorkersEvidenceSingle extends React.Component {
	constructor(props) {
	    super(props);
	    this.state = {
	    	toogleEdit: false,
	...
	handleRemove(){
		swal({
		  ...
		  html: false
			...
			Meteor.call('evidence.remove', data._id, (err)=>{
				if(err){
	...
	handleSave(e){
		...
		Meteor.call('evidence.update', data._id, this.state.description , (err)=>{
				if(err){
					Bert.alert( err.message, 'danger', 'growl-top-right' );
				}else{
